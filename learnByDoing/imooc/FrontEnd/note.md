...
- 前后端的通信方式
  - 使用浏览器访问网页
    - 在浏览器地址栏输入网址，按下回车
  - HTML的特殊标签
    - 浏览器在解析HTML标签时，遇到一些特殊的标签，会像服务器再次请求
      - link/img/script/iframe
      - 同一个域名下只能由六个并发的请求
    - 还有一些标签，在浏览器解析的时候，不会向服务器发送请求，但是用户可以使用它们向服务器发送请求
      - a/from
  - Ajax和Fetch
...
- HTTP是什么：超文本传输协议
  - 超文本：原先一个个单一的文本，通过超链接将其联系起来。由原先单一的文本变成了可无限延申、扩展的超级文本、立体文本
  - HTML、JS、CSS、图片、字体、音频、视频等等文件，都是通过HTTP（超文本传输协议）在服务器和浏览器之间传输
  - 每一次前后端通信，前端需要主动向后端发送请求，后端接到前端的请求后，给出响应
  - HTTP是一个请求-响应协议
- HTTP请求响应的过程
...
- RESTful接口设计
  ...
  - 注册新用户，使用POST方法
    - <http://www.imooc.com/api/http/user?id=1>
  - 修改一个用户，使用POST方法
...
- 安全性
  - GET和POST都不安全
  - 发送密码或其他敏感信息时不要使用GET，主要是避免他人窥屏或通过历史记录找到密码   
...
- HTTP状态码的语义
  - 100~199消息：请求已经被接受，需要继续处理
    - websocket
  - 200~299成功
    - 200： OK
  - 300~399重定向
    - 301 Moved Permanently（永久重定向），重定向的位置会被浏览器缓存
    - 302 Moved Temporarily（临时重定向），重定向的位置不会被浏览器缓存
    - 304 Not Modified（重定向没有修改），浏览器向服务器发送请求确认缓存是否过期，304没过期
  - 400~499请求错误
    - 400 Bad Request
    - 404 Not Found
  - 500~599服务器错误
